// Package mcpserver implements the MCP stdio server: protocol handling
// for initialize / notifications/initialized / tools/list / tools/call.
//
// Tool implementations live in internal/tools and are wired in via
// RegisterTool.
package mcpserver
...
// ToolHandler is invoked when tools/call is received for a registered
// tool. The handler returns the response body as a string; the server
// wraps it in the MCP content envelope. Returning an error produces a
...
// RegisterTool registers a tool descriptor and its handler. Must be
// called before Serve.
...
// Serve reads requests in a loop until ctx is cancelled or stdin
// returns EOF.
func (s *Server) Serve(ctx context.Context) error {
	for {
		...
		if err := s.handle(ctx, line); err != nil {
			...
			// intentionally stays silent. An error here means the
			// write itself failed; log and continue.
